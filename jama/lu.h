...
template <class Real> class LU {
  ...
  TNT::Array2D<Real> permute_copy(const TNT::Array2D<Real> &A,
                                  const TNT::Array1D<int> &piv, int j0,
                                  int j1) const {
    int piv_length = piv.dim();

    TNT::Array2D<Real> X(piv_length, j1 - j0 + 1);
  ...
  TNT::Array1D<Real> permute_copy(const TNT::Array1D<Real> &A,
                                  const TNT::Array1D<int> &piv) const {
    int piv_length = piv.dim();
    if (piv_length != A.dim())
      return TNT::Array1D<Real>();

    TNT::Array1D<Real> x(piv_length);
...
public:
  ...
  */
  ...
  LU(const TNT::Array2D<Real> &A)
      : LU_(A.copy()), m(A.dim1()), n(A.dim2()), piv(A.dim1())
  ...
  */
  ...
  TNT::Array1D<int> getPivot() { return piv; }
  ...
  */
  ...
  /** Solve A*x = b, where x and b are vectors of length equal
      ...
      of A.
      @return x a vector (TNT::Array1D> so that L*U*x = b(piv), if B is
     nonconformant,
